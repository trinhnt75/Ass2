...
import { addItem, getById, update } from "../../../../api/product";
...
const Additemm: React.FC = () => {
  ...
  const onSubmit = async (data: addForm) => {
    try {
      const reponse = await addItem(data);
      console.log(reponse);
      navigate("/admin");
    } catch (error) {
      console.log(error);
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-10 mx-auto items-center">
                <div className="col-span-1">
                  <input
                    type="file"
                    className="block w-full text-sm text-gray-500 rounded-lg border bg-white border-gray-200 p-20 text-sm
